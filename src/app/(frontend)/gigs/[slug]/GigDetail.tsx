...
export default function GigDetail({ initialData, serverURL }: { initialData: any; serverURL: string }) {
  const { data: gig } = useLivePreview({
    initialData,
    serverURL,
    depth: 1,
  })
  ...
  const tags: string[] = Array.isArray(gig.tags) ? gig.tags.filter(Boolean) : []
